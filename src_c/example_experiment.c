/**
 * This is a modified version of the original "example_experiment.c" to run the Hooke-Jeeves method and MTS-LS1 on the BBOB function set.
 *
 */
#include <math.h>
#include <stdlib.h>
 ...
 */
static const unsigned int BUDGET_MULTIPLIER = 1*1e4;
...
void example_experiment(const char *suite_name,
			...
			const int target_fun_id);

void mtsls1(evaluate_function_t evaluate_func,
...
void hooke_jeeves(evaluate_function_t evaluate_func,
                      evaluate_function_t evaluate_cons,
                      const size_t dimension,
                      const size_t number_of_objectives,
                      const size_t number_of_constraints,
...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[1], "-fun_id")) {
    target_fun_id = atoi(argv[1+1]);
  }
  ...
  example_experiment("bbob-largescale", "", "bbob-largescale", "result_folder: HJ-5", random_generator, target_fun_id);  
...
void example_experiment(const char *suite_name,
  ...
  while ((PROBLEM = coco_suite_get_next_problem(suite, observer)) != NULL) {
	...
	hooke_jeeves(evaluate_function,
		     evaluate_constraint,
		     dimension,
...
void mtsls1(evaluate_function_t evaluate_func,
  ...
  double *u = coco_allocate_vector(dimension);
  /* It is assumed that upper_bounds[j] and lower_bounds[j] takes the same value for each j. */
  double initial_step_size = 0.4;
  double step_size = initial_step_size;
  double learning_rate = 0.5;
  
  int number_of_evals = 0;
  double fun_value;
  double trial_fun_value;
  double prev_fun_value;
  ...
  fun_value = functions_values[0];
  number_of_evals++;
  
  while (number_of_evals < max_budget) { 
    prev_fun_value = fun_value;
          
    for (i = 0; i < dimension; ++i) {
      for (j = 0; j < dimension; ++j) u[j] = x[j];

      u[i] = x[i] - step_size * (upper_bounds[i] - lower_bounds[i]);
      /* Repair the solution*/
      if (u[i] > upper_bounds[i]) u[i] = upper_bounds[i];
      else if (u[i] < lower_bounds[i]) u[i] = lower_bounds[i];
      
      evaluate_func(u, functions_values);  
      trial_fun_value = functions_values[0];
      number_of_evals++;

      if (trial_fun_value < fun_value) {
	fun_value = trial_fun_value;
	x[i] = u[i];	 
      }
      else {
	u[i] = x[i] + 0.5 * step_size * (upper_bounds[i] - lower_bounds[i]);
    ...
    if (fabs(prev_fun_value - fun_value) <= 1e-8) {      
      step_size *= learning_rate;
      if (step_size * (upper_bounds[0] - lower_bounds[0]) < 1e-15) {
...
void hooke_jeeves(evaluate_function_t evaluate_func,
  ...
  double *u = coco_allocate_vector(dimension);
  double *x_prev = coco_allocate_vector(dimension);
  /* It is assumed that upper_bounds[j] and lower_bounds[j] takes the same value for each j. */
  double step_size = 0.4;
  double learning_rate = 0.5;
  double pattern_move_factor = 1;
  
  /* double step_size = 0.4 * (upper_bounds[0] - lower_bounds[0]);  */
  int number_of_evals = 0;
  double fun_value;
  double trial_fun_value;
  double prev_fun_value;
  ...
  while (number_of_evals < max_budget) { 
    prev_fun_value = fun_value;
    for (j = 0; j < dimension; ++j) x_prev[j] = x[j];
    ...
    if (fun_value < prev_fun_value) {      
      for (j = 0; j < dimension; ++j) u[j] = x[j] + pattern_move_factor * (x[j] - x_prev[j]);
      /* Repair the solution*/
      if (u[i] > upper_bounds[i]) u[i] = upper_bounds[i];
      else if (u[i] < lower_bounds[i]) u[i] = lower_bounds[i];
      evaluate_func(u, functions_values);
